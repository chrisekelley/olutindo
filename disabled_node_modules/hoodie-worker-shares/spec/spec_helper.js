...
JasminePromiseMatchers.toBeRejectedWith = function() {
  ...
  this.message = function() {
    if (fail.callCount === 0) {
      return ["Expected spy " + fail.identity + " to have been rejected with " + jasmine.pp(expectedArgs) + " but it was never resolved.", "Expected spy " + fail.identity + " not to have been rejected with " + jasmine.pp(expectedArgs) + " but it was."];
    } else {
      return ["Expected spy " + fail.identity + " to have been rejected with " + jasmine.pp(expectedArgs) + " but was resolved with " + jasmine.pp(fail.argsForCall), "Expected spy " + fail.identity + " not to have been rejected with " + jasmine.pp(expectedArgs) + " but was resolved with " + jasmine.pp(fail.argsForCall)];
    }
  };
